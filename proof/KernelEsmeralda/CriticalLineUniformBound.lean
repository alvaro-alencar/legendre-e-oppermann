...
theorem criticalLineGeometricFactor_le_four
  ...
  calc
    ...
    _ ≤ 4 := by
      push_cast
      rw [div_eq_mul_inv]
      have htwo : (((n + 1 : Nat) : Real)) ≤ 2 * (n : Real) := by
        push_cast
        linarith
      have hinv : 0 ≤ ((n : Real)⁻¹) := inv_nonneg.mpr hnpos.le
      have hm := mul_le_mul_of_nonneg_right htwo hinv
      have hnin : (n : Real) * (n : Real)⁻¹ = 1 := by field_simp [hnpos.ne']
      nlinarith
